...
当前版本的 `indoors` 修复并不是直接交换整个 `FarmHouse` 节点与客机小屋 `indoors` 节点，而是采用“**保留房屋身份节点，仅交换室内内容子节点**”的方式实现。

实现流程大致如下：

1. 在主客角色主体交换完成后，先读取交换前目标客机原本的 `homeLocation`
2. 在 `SaveGame/locations` 中定位：
   - 主屋：`GameLocation[name=FarmHouse]`
   - 目标客机原小屋：`GameLocation[name=Farm] / buildings / Building / indoors`
3. 通过客机交换前的 `homeLocation`，匹配对应 `indoors/uniqueName`，从而找到该客机实际绑定的小屋室内节点
4. 仅交换两处房屋内部与布局相关的子节点，而不交换整个房屋节点本身

需要注意的是：

- `FarmHouse` 与 `Cabin/indoors` 在结构上并不完全相同
- `Cabin/indoors` 还带有与客机归属绑定相关的字段，例如：
  - `uniqueName`
  - `farmhandReference`
- 如果直接整块交换整个房屋节点，容易破坏房屋类型和玩家归属关系

因此当前实现会保留以下字段在原房屋节点上不动：
...
也就是说，当前 `indoors` 修复的目标是：

- 让交换后的新主机进入 `FarmHouse` 时，看到的是原目标客机对应房屋的室内物品与布局
- 让降级为客机的原主机进入目标小屋时，看到的是原 `FarmHouse` 的室内物品与布局
- 同时尽量不破坏 `Cabin` 与客机之间的原有绑定结构

在实现方式上，主客角色主体交换仍采用原始 XML 内部内容替换；  
而 `indoors` 修复部分则使用 `ElementTree` 对整棵 XML 树做定点节点交换。
